...
class InteragentRenderTest < Minitest::Test
  ...
  def test_render_for_toc
    ...
    markdown = Prmd.render(schema, template: template, toc: true)

    assert_match /^# The table of contents/, markdown
    assert_match '<a href="#resource-app"', markdown
    assert_match '- <a href="#resource-app/POST/apps">POST /apps', markdown
    assert_match '<a name="resource-app/POST/apps"', markdown
  end
